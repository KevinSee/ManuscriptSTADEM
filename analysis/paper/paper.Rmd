...
<!-- John Hargrove: While I think this is a noteworthy application of STADEM, as written there is no justification of why you did this or how these values compare to other estimates made at LGR. In other words, were you interested in trends in abundance? Estimated coefficients of variation? Performance relative to another estimation procedure? 

We could do a comparison with the total window counts, or a comparison with SCOBI results, but I don't know that either is terribly relevant. SCOBI has been replaced by STADEM, I think. Comparing window counts with STADEM estimates of total unique fish, we could summarise the RMSE (~ 5000 for Chinook, ~10,000 for steelhead) or the normalized RMSE (~ 6% for Chinook, ! 7% for steelhead). Seem worth doing? Either that, or just explicitly state that we're doing a case study of STADEM at Lower Granite? -->
...
Examining the weekly estimates of nighttime passage and re-ascension rates at Lower Granite Dam (Figure \@ref(fig:night-reasc-diff-fig)), the two rates did not match up in most weeks. In particular, there are several weeks when the window counts are quite high, and the rates differ by as much as 10%. When nighttime passage is larger than re-ascension, the window counts will be biased low, and vice versa. 
...
```{r jags-fit-fig, fig.cap = "Time-series plot showing estimates of total escapement for Chinook in 2014, including raw window counts, window counts adjusted for nighttime passage, trap estimates and STADEM estimates. Gray ribbon represents the 95% credible interval for STADEM estimates."}
