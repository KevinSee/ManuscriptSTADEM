...
SimulateLGRdata = function(N.lgr = 100000,
  ...
  trap.rate.df %<>%
    dplyr::rename(trap_open = trap.open)
  ...
  if(hnc.prob >0){
    ...
    hnc.pop.p <- gtools::rdirichlet(1, alpha = rbinom(n.pops,1,hnc.pop.prob))
...
SummariseWeekly = function(sim_params,
  ...
  org_week = org_truth %>%
    ...
    left_join(org_truth %>%
                group_by(Week) %>%
                summarise(across(c(Night.passage, Day.passage, Window.passage, Fallback, Reascent, Trap, Ladder),
                                 sum),
                          .groups = "drop")) %>%
    left_join(org_truth %>%
                filter(Marked == 1,
                       Ladder == 1) %>%
                group_by(Week) %>%
                summarise(across(c(Reascent, Night.passage),
                                 sum),
                          .groups = "drop") %>%
                dplyr::rename(mark_reasc = Reascent,
...
SimulateLGRobs = function(sim_params,
  ...
  if(!perfect.window) {
    win_cnt_week = lgr_truth %>%
      group_by(Day, Week) %>%
      summarise(across(Window.passage,
                       sum),
                .groups = "drop") %>%
  ...
  caphist_list = trap_mr %>%
    split(list(.$Week)) %>%
    purrr::map(.f = capHistConvert,
               cols2use = c('M', 'C', 'R'),
               in.type = 'individual',
               out.type = 'frequency')

  # estimate trap rate based on mark-recapture
  trap_rate_mr = purrr::map_df(caphist_list,
                               .id = 'Week',
                               .f = function(x) {
                                 ...
                                 return(tibble(N_hat = mod$results['Mt', 'abundance'],
  ...
  lgr_week_obs %<>%
    mutate(across(c(Ladder:HNC_fish),
                  tidyr::replace_na,
                  replace = 0)) %>%
